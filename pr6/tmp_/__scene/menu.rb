# -*- encoding: UTF-8 -*-
#
require "__tewi/req"  if $0 ==__FILE__
#
# 2015-10-24 06:17:29 +0900
# ---------------------------------------- menu ----------------------------------------
module AnneRose
  class Menu
    attr_accessor :node_self, :font
    def initialize _
      @node_self = _
      @font      = Font.new(30, "Comic Sans MS" )
      node_self.Task :default_code do |o|  o.Code do
        node_self.Frandoll << :MSG_CODE1 if Input.keyPush? K_1
        node_self.Frandoll << :MSG_CODE2 if Input.keyPush? K_2
        node_self.up.delete              if Input.keyPush? K_9
        node_self.Scarlet = { :test_data => "test_data_by:#{node_self.sym}" }
      end end
      main
    end # initialize
    def main
       node_self.Task :nyaxxxx do |o|  o.Code do
         Window.drawFont(50,  10, "--menu--"   , @font)
         if Input.keyPush? K_Z

         end
		  end	end # o
    end # init

  end # menu
end # m
