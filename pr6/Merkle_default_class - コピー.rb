...
class Merkle_default_class < Game_obj
  def initialize _ , name
    @node_self    = _
    @scene_name   = name
    @load_dir     = "AnneRose"
  ...
  def __scene_code
    ...
    @node_self.Task :__debug_code do |o| o.Code do
      ...
      @node_self.Flandoll << :obj_user0     if Input.keyPush? K_4
...
#--------- メソッド内でrefine使えない感じなので普通にオーバーライド ------------
    __Window_refine_do
      ...
      when -> rb {  File.exists?(rb) && load(rb) }
        ...
        @node_self.Task :__load_src_error do |o| o.Code do
          ...
          Window.drawFont(50,  10, "--#{@load_dir}/#{@scene_name}--" , Font.default)
        end end
      end
    __Window_refine_end
  end # def
  def self.loading o , rb
    if rb =~ /obj_/
      Merkle_default_class.new o , rb , true
      return true
    end
