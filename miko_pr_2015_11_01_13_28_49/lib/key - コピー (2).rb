...
module Create_VK_action_replay
  def self.arg profile , save_file
    flame = 0 ; define_method :VK_Replay_update do flame+=1 end
...
module Create_VK_action_save
  def self.arg profile , save_file
    ...
    p keys_hash
    p "----------------------------------"
   x=  DXRuby.constants.inject({}){|r,s|s=~/K_|P_/&&r[s]=DXRuby.const_get(s);r}
    ...
    [:VK_Push?,:VK_Down?].each do |me|
      define_method me do |vk|
        ...
        tf = key_data[ profile ][ vk ].any? do | m |
          Input.keys.any? do |s|
            keys_hash[s] == m.to_sym
          end
        end
        save.call(profile, vk) if tf
        tf
      end
    end if nil
...
# o =  Object.new
# o.extend Create_VK_action_replay.arg(:profile0)
# o.VK_Push? :VK_0
#
# exit
...
o =  Object.new
o.extend Create_VK_action_save.arg(:profile0,"./dat/replay/replay1.yaml")
